...
import subsystem.Users;
import subsystem.Direction;
import subsystem.Disk;
import subsystem.DiskBlock;
import subsystem.File;

public class Main {

	public static void main(String[] args) throws IOException {
		System.out.println("欢迎光临文件系统，请输入您的用户名：");
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		String username;
		while ((username = br.readLine().trim()).equals(""))
			System.out.println("用户名输入有误，请重新输入");
		Direction nowdir = new Direction(username);
		System.out.println("欢迎光临" + username + "您可以创建新的目录和文件，如需帮助，输入help查看帮助指令");
		Direction dirsave = null;
		File filesave = null;
		while (true) {
			...
			if (s.equals("ls"))        // --->目录显示,1
				...
				if (a != null) {
					...
					if (nowdir.canPasteFile(new File(newname))) {       //判断是否存在同名文件
						if (!newname.equals("")) {
							nowdir.deleteFile(real);
							File thnew = a;
				...
				if (a != null) {
					...
					int point = 0;
					ArrayList<DiskBlock> newFileBlocklist = new ArrayList<DiskBlock>();

					ArrayList<Integer> thenw = new ArrayList<Integer>();
					int blocksize = DiskBlock.getSize();
					int sizecount = 0;
					if (realcontent.length() > blocksize) {
						for (int i = 0; i < realcontent.length() - blocksize; i = i
							...
							Disk.getInstance().addUsed(newblock);
						...
						Disk.getInstance().addUsed(ano);        //修改后的内容保存到磁盘
						thenw.add(ano.getId());
				...
				if (filesave != null) {
					ArrayList<DiskBlock> fileblocklist = filesave
							.getBlocklist();
					if (nowdir.canPasteFile(filesave)) {        //判断目录下是否有同名文件
						StringBuffer realcontent = new StringBuffer();
						for (DiskBlock one : fileblocklist) {
							realcontent.append(one.getContent());        //先将文件中的StringBuffer全部拷贝出来
						}

						int point = 0;
						ArrayList<DiskBlock> newFileBlocklist = new ArrayList<DiskBlock>();        //创建新的磁盘块列表用来存放这个StringBuffer
						int blocksize = DiskBlock.getSize();
						int sizecount = 0;
						ArrayList<Integer> thenw = new ArrayList<Integer>();
						if (realcontent.length() > blocksize) {
							...
							sizecount++;
							DiskBlock ano = new DiskBlock(rest);
							Disk.getInstance().addUsed(ano);          //修改后的内容保存到磁盘,一定要先保存到磁盘,在分配好磁盘号之后,再保存到文件
							thenw.add(ano.getId());
							newFileBlocklist.add(ano);          //保存到文件
						} else {
							StringBuffer shortone = new StringBuffer(
						...
						File anoth = new File(filesave.getName());         
						//新建一个文件用来保存复制板里的信息,不要直接用filesave,之前在此处出错,debug用时2小时,就是因为没new一个新的File
				...
				String real = s.substring(7, s.length());
				Direction old = nowdir.getDir(real);
				if (old != null) {
					System.out.println("请输入新的目录名");
					String newname = br.readLine().trim();
					if (nowdir.canPasteDir(new Direction(newname))) {          //确认该目录下无同名目录
						if (newname != "") {
				...
				Direction dir = new Direction(dirname, 0, 0);
				nowdir.addDir(dir);

			} else if (s.startsWith("cpdir") && s.length() > 6) {         // 复制目录
				String real = s.substring(6, s.length());
				Direction a = nowdir.getDir(real);
				if (a != null) {
					dirsave=new Direction();
					dirsave = (Direction) Direction.cloneObject(a);
				...
				if (dirsave != null) {
					...
					instance = (Direction) Direction.cloneObject(dirsave);
					if (nowdir.canPasteDir(instance)) {          //检查无同名目录，才能添加
						...
						ArrayList<DiskBlock> bridge = new ArrayList<DiskBlock>();
						for (int i = 0; i < oldused.size(); i++) {
							DiskBlock a = new DiskBlock();
							Disk.getInstance().addUsed(a);
							thenew.add(a.getId());
						}
				...
				Disk.getInstance().showUsed();
			} else if (s.equals("help")) {         // 显示帮助，1
				System.out.println("-------------------------以下为显示和跳转操作------------------------------");
				System.out.println("ls   显示文件目录             cd [dirname]:   跳转到指定目录                cd ..跳转到上层目录        ");
		        System.out.println("---------------------------以下为目录操作--------------------------------");
		        System.out.println("mkdir [dirname]:创建目录       ptdir [dirname]:粘贴目录       cpdir [dirname]:拷贝目录 ");
		        System.out.println("rnmdir [dirname]:重命名目录            rmdir [dirname]:删除目录");
		        System.out.println("---------------------------以下为文件操作--------------------------------");
		        System.out.println("vim [filename]:创建文件       ptfile [filename]:粘贴文件       cpfile [filename]:拷贝文件 ");
		        System.out.println("rnmfile [filename]:重命名文件            rmfile [filename]:删除文件       vi [filename]:显示文件内容");
		        System.out.println("open 打开文件     之后输入正确的文件名就能打开进行编辑了");
		        System.out.println("---------------------------以下为磁盘操作--------------------------------");
		        System.out.println("showdisk  :显示磁盘使用情况");
		        System.out.println("---------------------------以下为退出该系统操作----------------------------");
				...
				Direction thisone = new Direction();
				thisone = nowdir;
				while (nowdir.getFatherDir() != null)        //循环更新父目录磁盘块(添加)和大小，直至根目录
				{
					brid.push(nowdir.getName()+"/");
               ...
               Users.getInstance().addUser(thisone);
               ...
               if(in.equals("exit"))  System.exit(0);
   				...
   				if(Users.getInstance().whetherExist(anouser)){
   					nowdir=Users.getInstance().getUserDir(anouser);
   					System.out.println("欢迎光临" + anouser + "您可以创建新的目录和文件，如需帮助，输入help查看帮助指令");
   	   				username=anouser;
   				}else System.out.println("不存在该用户");   					
               }
               else if (in.startsWith("add") && in.length() > 4) {        //创建新账号并登录
      				String newuser = in.substring(4, in.length());
      				if(!Users.getInstance().whetherExist(newuser)){
      					nowdir= new Direction(newuser);
          				System.out.println("欢迎光临" + newuser + "您可以创建新的目录和文件，如需帮助，输入help查看帮助指令");
          					Users.getInstance().addUser(nowdir);
          					username=newuser;
      				}
